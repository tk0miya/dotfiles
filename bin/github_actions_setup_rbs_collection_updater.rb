...
PRIVATE_KEY_PATH = File.expand_path("~/Dropbox/Personal/secrets/rbs_collection_updater.private-key.pem", __dir__)
...
def get_public_key(repo_name)
  path = "/repos/#{repo_name}/actions/secrets/public-key"
...
def print_github_token_error
  ...
  puts "1. Generate a GitHub Personal Access Token (fine-grained) at:"
  puts "   https://github.com/settings/personal-access-tokens"
  puts "2. Set the following permissions under 'Repository permissions':"
  puts "   - Actions variables and secrets: Read and write"
  puts "3. Set the environment variable: export GITHUB_TOKEN='your-token'"
  exit 1
end
...
def main
  ...
  set_action_variable(repo_name, "RBS_COLLECTION_UPDATER_APP_ID", "412513")

  # Set up Actions Secret
  puts "Setting up Actions Secret..."
  private_key = File.read(PRIVATE_KEY_PATH)
  set_action_secret(repo_name, "RBS_COLLECTION_UPDATER_PRIVATE_KEY", private_key)
