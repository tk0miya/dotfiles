...
if [ `hostname` = "deneb" ]; then
    REGULAR_PYTHON_VERSIONS="2.7.11 2.6.9 3.5.1 3.4.3 3.3.6"
elif [ `hostname` = "capella" ]; then
    REGULAR_PYTHON_VERSIONS="2.7.11 3.5.1"
else
    REGULAR_PYTHON_VERSIONS="2.7.11"
fi
...
eval "$(rbenv init -)"
for version in 1.9.3-p551 2.2.4; do
...
eval "$(ndenv init -)"
for version in v0.12.9 v5.5.0; do
